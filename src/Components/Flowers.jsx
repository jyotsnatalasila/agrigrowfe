...
function FlowersContent() {
  ...
  const toggleWishlist = (flower) => {
    if (isInWishlist(flower.id)) {
      removeFromWishlistGlobal(flower.id);
    } else {
      addToWishlistGlobal(flower);
    }
  };
  ...
  const openModal = (flower) => {
    ...
    setCurrentRating(0);
    const defaultUnit = flower.priceOptions[0]?.unit || null;
  ...
  const sortedFlowers = useMemo(() => {
    const flowersWithDefaultPrice = Flowers.map(flower => ({
      ...flower,
      itemPrice: flower.priceOptions[0].unitPrice
    }));

    if (sortOption === "default") return [...flowersWithDefaultPrice];

    let sorted = [...flowersWithDefaultPrice];
        ...
        >
          {sortedFlowers.map((flower) => (
            <div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => e.key === "Enter" && openModal(flower)}
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  fontSize: "24px",
                  color: isInWishlist(flower.id) ? "#8dc63f" : "#999",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
              ...
              >
                Quantity: {formatUnitLabel(flower.priceOptions[0].unit, 1, flower.priceOptions[0].gramWeight)}
              </div>
              ...
              >
                ₹{flower.priceOptions[0].unitPrice}
              </div>
            </div>
          ))}
        </div>
