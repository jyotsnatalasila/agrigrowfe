...
function AquaticFeedContent() {
  ...
  const toggleWishlist = (feed) => {
    if (isInWishlist(feed.id)) {
      removeFromWishlistGlobal(feed.id);
    } else {
      addToWishlistGlobal(feed);
    }
  };
  ...
  const openModal = (feed) => {
    ...
    setCurrentRating(0);
    const defaultUnit = feed.priceOptions[0]?.unit || null;
  ...
  const sortedFeed = useMemo(() => {
    const feedWithDefaultPrice = AquaticFeed.map(feed => ({
      ...feed,
      itemPrice: feed.priceOptions[0].unitPrice
    }));

    if (sortOption === "default") return [...feedWithDefaultPrice];

    let sorted = [...feedWithDefaultPrice];
        ...
        >
          {sortedFeed.map((feed) => (
            <div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => e.key === "Enter" && openModal(feed)}
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  fontSize: "24px",
                  color: isInWishlist(feed.id) ? "#8dc63f" : "#999",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
              ...
              >
                Quantity: {formatUnitLabel(feed.priceOptions[0].unit, 1, feed.priceOptions[0].gramWeight)}
              </div>
...
export default function AquaticFeedPage() {
  return <AquaticFeedContent />;
}
